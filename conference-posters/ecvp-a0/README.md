...
- `qr-destination.txt` – canonical interactive-demo URL encoded by the QR.
- `render.sh` – reproducible local render.
- `assets/vorogaze-demo-qr.svg` – generated vector QR.
...
The poster uses the bundled Lisa1 face fixture and descriptive outputs produced
by the current VoroGaze AOI demo helpers. It deliberately describes a
research-methods prototype and worked demonstration, not a completed empirical
validation study.
...
The QR visibly targets the public, bundled-data interactive demo:

```text
https://vorogaze-demo.mjgreen.uk/
```

The separate static project page is printed on the poster as:
